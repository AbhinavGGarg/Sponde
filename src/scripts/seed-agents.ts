...
- When you and the counterpart have converged, one side posts kind "accept" restating the exact final terms; then BOTH sides call commit_deal with those identical terms, one short sentence — plus starts_at (ISO-8601 with timezone offset), duration_minutes, and location when the agreement has a time and place, so the sealed room can issue a correct calendar hold. commit_deal will pause for your human's approval — that is correct and expected. Never try to finalize any other way.
